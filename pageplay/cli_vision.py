...
但不进框选交互——直接整页截图交识别引擎（vision.screenshot_table），
识别结果走 actions.save_table 落 CSV+JSON。是 DOM 框选之外的第二条
取数路：canvas 渲染、图片表格等 DOM 读不出的场景交给引擎看。

引擎（T22 起本地库优先）：--engine local 缺省，离线本地 OCR
（img2table/rapidocr，不出网零成本）；--engine glm 走 GLM 视觉模型，
需 GLM_API_KEY 环境变量。
...
GLM_API_KEY 未设 / 识别引擎异常时调用向上抛，由 main 统一映射
人话与退出码（配置问题 ValueError → 1；服务异常 RuntimeError → 1；
真风控 RiskTriggered → 2）。退出码：识别到表格并落盘 0；引擎说没有
...
def _cmd_vision(args: argparse.Namespace) -> int:
    """vision：附着活窗取当前页，截屏交识别引擎，结果落盘落账。"""
    ...
    engine = str(args.engine or "local").strip().lower()  # glm 之外的值
    engine_label = "GLM 视觉模型" if engine == "glm" else "本地 OCR（离线）"
    # 由 vision.screenshot_table 校验并报 ValueError → main 人话
    print(f"将截取该页画面交给{engine_label}识别：{title or '（无标题）'}"
          f"（{page.url[:60]}）")

    rows = vision.screenshot_table(page, engine=engine)  # 缺 key(仅 glm)/异常 → main 人话
    if not rows:
        print(f"✗ {engine_label}没有识别到表格"
              "（确认页面里真的有表格；没有表格的页不属于视觉抓取）")
        _record_run(f"{label}-vision", "vision", "fail",
                    f"{engine_label}没有识别到表格", [])
        return 1

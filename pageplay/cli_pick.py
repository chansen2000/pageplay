...
确认即执行：pick 每确认一条走 confirm_and_execute（存 recipe → 当场
执行 → ✓/✗ → 落账 → 截封面），单条失败不中断会话；会话里出现"提取到
0 行"（锁错目标，T20 不再当成功）→ 会话结束退出码 1。pick 以
repeat=True 跑会话（T9a/T9c），结束打印摘要。record（T11d）录整段浏览
为流程，收获步当场执行落账但不存单条 recipe。run 双查流程/recipe 重放；
遇登录反弹进风控协作等人，通过后自动续跑；results 读账本回看。
"""
...
def _runs_path() -> Path:
    """执行账本路径：PAGEPLAY_HOME 覆盖，默认 ~/.pageplay/runs.jsonl（跨站统一一份）。"""
...
def _save_cover(page, result: dict, site_dir: Path, name: str) -> None:
    ...
    try:
        picker.cover_screenshot(
            page, result["selector"], site_dir / "recipes" / f"{name}.png")
...
def _execute_and_record(page, name: str, action: str, selector: str,
    ...
    runs.jsonl（ledger_action 给定时账本 action 记它——如 grab）。真
    风控 RiskTriggered 不吞、向上穿透（main 映射退出码 2）；其余失败
    不向上抛：单条失败不打断框选/录制会话。成功返回 (产物列表, "")，
...
def _goto_with_login_recovery(page, browser, site, url: str, max_retries: int = 2) -> str | None:
    """goto 目标页；被弹回登录页 → 风控协作等人，通过后自动续跑。

    每轮：goto → 反弹判定（host 含 login.）→ 未弹回再过 Guard（Guard
    命中且落在登录页同样进协作；真风控照旧上抛停机）。弹回则等人完成
    登录/验证（至多 120s）后重试，上限 max_retries 次。成功返回 None。
    """
...
def _cmd_pick(args: argparse.Namespace) -> int:
    ...
    结束（关窗/空闲超时/Ctrl-C）打印摘要；一条没收（Esc/关窗）→ 已取消
    退出码 1；会话出现过"提取到 0 行"（锁错目标，T20）→ 退出码 1。
    """
    try:
    ...
    names: list[str] = []
    counter = [0]
    zero = [False]  # 会话内出现过 0 行提取 → 结束退出码 1（会话照常继续）

    def _handler(result: dict) -> None:
        counter[0] += 1
        name = base_name if counter[0] == 1 else f"{base_name}-{counter[0]}"
        names.append(name)
        detail = confirm_and_execute(page, site, site_dir, name, result)
        if detail and detail.startswith(actions.ZERO_ROWS_PREFIX):
            zero[0] = True
    ...
    return 1 if zero[0] else 0


def _cmd_record(args: argparse.Namespace) -> int:
    """record：附着有头活窗录制整段浏览为流程（T11d，设计 §12）。

    自然浏览点击照常放行、自动记账（recorder.py 契约）；按 P 进框选产生
    收获步，当场执行落账但不存单条 recipe，流程才是归属。结束（关窗/
    超时/Ctrl-C）给流程起名落盘 flows/<名字>.json，以后 pageplay run
    整链重放；录制期产物放占位目录。一条没收（PickCancelled）→ 退出码 1。
    """
    try:
        _name, pasted_url = _cli.parse_target(args.site)
        site = _cli._resolve_target(args.site)
...
def _run_flow_by_name(args: argparse.Namespace, flow_dir: Path) -> int:
    ...
    用"执行前后目录快照对比"得出。登录反弹协作、逐步重试语义全在 runner。
    """
    name = str(args.name)
    ...
    outputs = sorted(Path(p) for p, sig in _snapshot_dir_files(out_dir).items()
                     if before.get(p) != sig)
    steps_line = "；".join(
        f"第{r['no']}步[{r['kind']}]{'✓' if r['status'] == 'ok' else '✗'}：{r['detail']}"
...
def _cmd_run(args: argparse.Namespace) -> int:
    ...
    流程命中 → runner.run_flow 整链重放；recipe 命中 → 单条重放（T9b
    语义零回归）。被弹回登录页都不直接停：进风控协作等人，通过后自动
    续跑（重试上限 2 次）。两者都找不到 → 人话列现有流程/recipe；
    结果无论成败落账 runs.jsonl。
    """
    ...
    if flow_dir is None and site_dir is None:
        ...
        print(f"{args.name!r} 既不是已保存的流程，也不是 recipe；现有流程：{flow_known}；"
              f"现有 recipe：{recipe_known}", file=sys.stderr)
        return 1
    print("── 执行步骤 ──\n1. 已按流程逐步执行\n"
          "2. 被弹登录页时去窗口里过一下验证\n3. 产物路径最后列出\n─────────────")
